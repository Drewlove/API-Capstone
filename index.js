...
const CO2EMISSIONSPERGDPLABEL =
  "GDP (billions of current dollars) per 1 MMT of CO2 Emissions";
...
function renderLandingPage() {
  let landingPage = `<section class='landing'>
...
function renderForm() {
  ...
  globalState = {};
  emptyMainContent();
  let compareStateForm = `<section id='page-compare'>
    <form>
    <fieldset>
...
function getStateOptions() {
  ...
  return stateNames
    .map((key) => {
      return `<option value=${key.toString()}>${
        stateListMaster[key].name
      }</option>`;
    })
    .join("");
}
...
function getCO2emissionsByEnergyType(energy) {
  ...
  energy.forEach((eType) => {
    ...
    energyCO2emissionsObj.energyType = {
      ...energyCO2emissionsObj.energyType,
      ...energyProp,
    };
  });
...
function fetchTotalCO2data() {
  ...
  Promise.all(
    urls.map((url) =>
      fetch(url).then(checkStatus).then(parseJSON).then(formatFetchedData)
    )
  )
...
function getUrls(stateAbbrs, energy) {
  ...
  stateAbbrs.forEach((state) => {
    energy.map((energyType) => {
      return urlList.push(
        `https://developer.nrel.gov/api/cleap/v1/state_co2_emissions?state_abbr=${state}&type=${energyType}&api_key=${apiKey}`
      );
    });
  });
  return urlList;
...
function getStateCO2byEnergyType(responseArr) {
  ...
  responseArr.forEach((response) => {
    ...
    stateCO2byEnergyType[state] = {
      ...stateCO2byEnergyType[state],
      [energy]: response[key[0]],
    };
  });
...
function getStateGdpPerCO2(stateName) {
  ...
  years.forEach((year) => {
    gdpPerCO2emissions[year] = getGdpEmissionsRatio(
      gdpObj[year],
      year,
      totalCO2emissionsObj
    );
  });
...
function energyTypeContent(energy) {
  return `<div><h2>${energy
    .map((energy) => capitalize(energy))
    .join(", ")}</h2></div>`;
}
...
function getGraphData(yAxisLabel, state, years) {
  let stateAbbr = stateListMaster[state].abbr;
  if (yAxisLabel === TOTALCO2EMISSIONSLABEL) {
    let totalCO2emissionsObj = sumCO2emissions(
      stateListMaster[state].abbr,
      years
    );
    return Object.keys(totalCO2emissionsObj).map(
      (year) => totalCO2emissionsObj[year]
    );
  } else if (yAxisLabel === CO2EMISSIONSPERGDPLABEL) {
